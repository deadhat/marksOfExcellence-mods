...
# Embedded file name: source\mod_marksOnGunExtended.py
# Compiled at: 2022-04-13 22:03:04
import datetime
import math
...
battleDamageRating = [battleDamageRating0,
 battleDamageRating20,
 battleDamageRating40,
 battleDamageRating55,
 battleDamageRating65,
 battleDamageRating85,
 battleDamageRating95,
 battleDamageRating100]
LEVELS = [0.0,
 20.0,
 40.0,
 55.0,
...
class Config(object):
    ...
    def __init__(self):
        ...
        self.buttons = {'buttonShow': [Keys.KEY_NUMPAD9, [Keys.KEY_LALT, Keys.KEY_RALT]],
        ...
        self.data = {'version': self.version_id,
         'enabled': True,
         ...
         'battleMessage{c_status}Up': '\xce\x94',
         ...
         'UI': 10}
        self.i18n = {'version': self.version_id,
         ...
         'UI_setting_UI_tooltip': '{HEADER}UI in battle{/HEADER}{BODY}Extended:<br/><img src="img://objects/ui_extended.png"></img><br/>Simple:<br/><img src="img://objects/ui_simple.png"></img><br/>Config:<br/>/mods/configs/marksOnGunExtended/marksOnGunExtended.json<br/>{/BODY}',
        ...
        return {'modDisplayName': self.i18n['UI_description'],
         ...
         'column1': [{'type': 'CheckBox',
                     ...
                     {'type': 'Dropdown',
                      ...
                      'options': [{'label': self.i18n['UI_menu_UIConfig']},
    ...
    def generator_menu(self):
        ...
        for i in xrange(len(COLOR)):
            res.append({'label': '<font color="%s">%s</font>' % (COLOR[i], self.i18n[MENU[i]])})
...
class Worker(object):
    ...
    def __init__(self):
        ...
        self.values = [0,
         ...
         0,
         0,
         datetime.datetime.toordinal(datetime.datetime.utcnow()) - 1,
         datetime.datetime.toordinal(datetime.datetime.utcnow()) - 1]
        self.name = ''
        self.dossier = None
        self.initiated = False
        self.replay = False
        self.formatStrings = {'status': '',
         'battleMarkOfGun': '',
         'currentMarkOfGun': '',
         'nextMarkOfGun': '',
         'damageCurrent': '',
        ...
        self.messages = {'battleMessageskill4ltu': '<font size="20">{c_battleMarkOfGun} ({currentMarkOfGun}){status}</font>\n',
    ...
    def keyPressed(self, event):
        ...
        if self.level and self.movingAvgDamage:
            ...
            if g_gui.get_key(config.data['buttonShow']) and isKeyDownTrigger:
                ...
                if config.data['UI'] > 10:
                    config.data['UI'] = 1
                status = [config.i18n['UI_menu_UIConfig'],
                 config.i18n['UI_menu_UIskill4ltu'],
                 config.i18n['UI_menu_UIMyp'],
                 config.i18n['UI_menu_UIspoter'],
                 config.i18n['UI_menu_UIcircon'],
                 config.i18n['UI_menu_UIReplay'],
                 config.i18n['UI_menu_UIReplayDamage'],
                 config.i18n['UI_menu_UIReplayColor'],
                 config.i18n['UI_menu_UIReplayColorDamage'],
                 config.i18n['UI_menu_UIoldskool'],
                 config.i18n['UI_menu_UIspoterNew']]
    ...
    def check_player_thread():
        player = BigWorld.player()
        return '%s' % player.databaseID if hasattr(player, 'databaseID') else '%s' % player.arena.vehicles[player.playerVehicleID]['accountDBID']
...
class Flash(object):
    ...
    def update(self, alias, props):
        if str(alias) == str(config.ids):
            ...
            self.setupSize()
            print '%s Flash coordinates updated : y = %i, x = %i, props: %s' % (alias,
             config.data['panel']['y'],
             config.data['panel']['x'],
             props)

    def setup(self):
        self.name = {COMPONENT_TYPE.LABEL: '%s' % config.ids}
        self.data = {COMPONENT_TYPE.LABEL: {'index': 10000,
                                'x': -226.0,
                                'y': -226.0,
                                'width': 183.0,
                                'height': 50.0,
    ...
    def screenResize(self):
        ...
        self.updateObject(COMPONENT_TYPE.LABEL, {'x': x,
         'y': y})
        return
